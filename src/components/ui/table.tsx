...
const TableHeader = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(({
  className,
  ...props
}, ref) => <thead ref={ref} className={cn("[&_tr]:border-b-0 mt-0", className)} {...props} />);
...
const TableHead = React.forwardRef<HTMLTableCellElement, TableHeadProps>(({
  ...
  children,
  ...props
}, ref) => <th ref={ref} className={cn("h-12 px-4 text-left align-middle font-medium text-muted-foreground [&:has([role=checkbox])]:pr-0", dragOver && "border-t-2 border-primary", sortable && "cursor-pointer select-none", dragging && "opacity-70 bg-accent shadow-md border border-primary", className)} onClick={sortable ? onSort : undefined} {...props}>
    <div className={cn("flex items-center gap-2", dragging && "pointer-events-none")}>
      {draggable && <span className="flex items-center cursor-grab opacity-50 hover:opacity-100 touch-none">
...
const TableContainer = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & {
  ...
  ...props
}, ref) => <div ref={ref} className={cn("relative border rounded-md overflow-hidden flex-1", "transition-all duration-300 shadow-sm", "w-full max-w-full", height, className)} style={{
  scrollbarWidth: 'thin',
