...
export function DeliveryTable({
  ...
  return <div className="flex-1 overflow-hidden px-px">
      <div className="flex h-full">
        <DeliverySidebar open={isFilterSidebarOpen} onClose={toggleFilterSidebar} deliveryStatuses={allDeliveryStatuses} selectedStatuses={selectedStatuses} onStatusChange={setSelectedStatuses} />
              ...
              <Table>
                <TableHeader className="sticky top-0 bg-background z-10">
                  <TableRow>
                    ...
                    return <TableHead key={columnId} draggable={true} onDragStart={e => handleDragStart(e, columnId)} onDragOver={e => handleDragOver(e, columnId)} onDragEnd={handleDragEnd} onDrop={e => handleDrop(e, columnId)} className={`${columnId === "distance" || columnId === "couriersEarnings" ? "text-right" : ""} whitespace-nowrap truncate max-w-[200px]`}>
                    ...
                    switch (columnId) {
                      case "status":
                        return <TableCell key={columnId}>
                                  <Badge variant={getStatusBadgeVariant(delivery.status) as any} className={`${delivery.status === "Dropoff Complete" ? "bg-green-100 text-green-800 hover:bg-green-100" : ""}`}>
                      ...
                      case "courier":
                        return <TableCell key={columnId}>
                                  {delivery.courier ? <Button variant="link" className="p-0 h-auto font-normal text-primary" onClick={() => onCourierClick(delivery.courier)}>
                                      {delivery.courier}
                                    </Button> : <span>-</span>}
                                </TableCell>;
                      case "organization":
