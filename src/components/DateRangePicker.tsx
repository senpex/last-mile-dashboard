...
export function DateRangePicker({
  ...
  const timeOptions = React.useMemo(() => {
    ...
    const periods = ["AM", "PM"];
    return hours.flatMap(hour => minutes.flatMap(minute => periods.map(period => `${hour}:${minute} ${period}`)));
  }, []);
        ...
        <div className="flex flex-col max-h-[500px] max-w-[500px]">
          ...
          <div className="flex">
            ...
            <div className="w-32 border-r p-2 max-h-[400px] overflow-y-auto">
              {datePresets.map(preset => <Button key={preset.label} variant="ghost" className="w-full justify-start text-left mb-1 text-xs h-8" onClick={() => {
              applyPreset(preset.days, preset.startOfDay || preset.startOfWeek || preset.startOfMonth, preset.startOfWeek ? 'week' : preset.startOfMonth ? 'month' : undefined);
            }}>
                  {preset.label}
                </Button>)}
            </div>
            ...
            <div className="p-2 flex-1">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Button variant="outline" className="w-full justify-start text-left text-xs h-8 mb-1" onClick={() => {}}>
                    {dateRange?.from ? format(dateRange.from, "MMM dd, yyyy") : <span>Pick a date</span>}
                  </Button>
                  ...
                  <Select value={startTime} onValueChange={setStartTime}>
                    ...
                    <SelectContent>
                      {timeOptions.map(time => <SelectItem key={time} value={time} className="text-xs">
                          {time}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div>
                  ...
                  <Button variant="outline" className="w-full justify-start text-left text-xs h-8 mb-1" onClick={() => {}}>
                    {dateRange?.to ? format(dateRange.to, "MMM dd, yyyy") : <span>Pick a date</span>}
                  </Button>
              ...
              <div className="mt-2">
                <Calendar mode="range" defaultMonth={dateRange?.from} selected={dateRange} onSelect={onDateRangeChange} numberOfMonths={1} showOutsideDays={false} className="rounded-md" />
              </div>
            </div>
          </div>
