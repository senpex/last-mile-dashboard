...
const Index = () => {
  ...
  // Initialize with some random users having messages
  useEffect(() => {
    // Randomly select couriers and customers who have messages
    const randomCustomers: string[] = [];
    const randomCouriers: string[] = [];
    
    deliveries.forEach(delivery => {
      if (delivery.customerName && Math.random() < 0.2) {
        randomCustomers.push(delivery.customerName);
      }
      if (delivery.courier && Math.random() < 0.2) {
        randomCouriers.push(delivery.courier);
      }
    });
    
    setUsersWithMessages({
      customers: [...new Set(randomCustomers)],
      couriers: [...new Set(randomCouriers)]
    });
  }, []);
