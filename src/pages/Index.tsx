...
// Define sorting types for type safety
type SortDirection = 'asc' | 'desc' | null;
type SortableField = 'packageId' | 'orderName' | 'status' | 'pickupTime' | 'dropoffTime' | 'price' | 'tip' | 'fees' | 'courier' | 'organization' | 'distance' | null;
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const customersWithMessages = deliveries
      .filter(d => d.dropoffLocation?.name)
      .filter(() => Math.random() < 0.25)
      .map(d => d.dropoffLocation.name);
    
    setUsersWithMessages({
      couriers: Array.from(new Set(couriersWithMessages)),
      customers: Array.from(new Set(customersWithMessages))
    });
  }, []);
