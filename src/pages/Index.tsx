...
const Index = () => {
  ...
  const deliveries = [
    ...
    },
    {
      ...
      dropoffLocation: {
        ...
        address: "512 NW Cherry Blossom Ave, Bentonville, AR 72712, US"
      },
      price: "$299.99",
      tip: "$30.00",
      fees: "$14.99",
      courier: "William Roberts",
  ...
  useEffect(() => {
    if (debouncedSearchTerm) {
      const filtered = deliveries.filter((delivery) => {
        return (
          delivery.packageId.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
          delivery.orderName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
          delivery.organization.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
          (delivery.courier && delivery.courier.toLowerCase().includes(debouncedSearchTerm.toLowerCase()))
        );
      });
  ...
  const [columns, setColumns] = useState<ColumnOption[]>([
    { id: "packageId", name: "Package ID", visible: true },
    { id: "orderName", name: "Order Name", visible: true },
    { id: "status", name: "Status", visible: true },
    { id: "organization", name: "Organization", visible: true },
    { id: "pickupTime", name: "Pickup Time", visible: true },
    { id: "pickupLocation", name: "Pickup Location", visible: false },
    { id: "dropoffTime", name: "Dropoff Time", visible: true },
    { id: "dropoffLocation", name: "Dropoff Location", visible: false },
    { id: "price", name: "Price", visible: true },
    { id: "tip", name: "Tip", visible: false },
    { id: "fees", name: "Fees", visible: false },
    { id: "courier", name: "Courier", visible: true },
    { id: "distance", name: "Distance", visible: false },
  ]);

  // Dragging handlers for column reordering
  ...
  const handleDrop = (e: React.DragEvent<HTMLTableCellElement>, targetColumnId: string) => {
    ...
    if (draggedColumn && draggedColumn !== targetColumnId) {
      ...
      const draggedColumnIndex = updatedColumns.findIndex(col => col.id === draggedColumn);
      ...
      updatedColumns.splice(targetColumnIndex, 0, draggedItem);
  ...
  const getStatusDisplay = useCallback((status: string) => {
    if (statusDictionary && statusDictionary.items) {
      const statusItem = statusDictionary.items.find(item => 
      ...
      if (statusItem) {
        return {
          ...
          color: statusItem.color || 'default'
        };
      }
    }
  ...
  const getTimezoneDisplay = () => {
    ...
    return currentTimezoneInfo ? `${currentTimezoneInfo.label} ${currentTimezoneInfo.offset}` : timezone;
  };
          ...
          <div className="flex flex-col space-y-6">
            ...
            <div className="bg-muted/30 px-4 py-2 rounded-md">
              <div className="text-sm">
                <span className="font-medium">Timezone:</span> {getTimezoneDisplay()}
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" size="icon" title="Filter Deliveries">
                      <Filter className="h-4 w-4" />
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent align="start" className="w-80">
                    <div className="space-y-4">
                      <h4 className="font-medium">Filter Deliveries</h4>
                      {/* Filter form would go here */}
                      <p className="text-sm text-muted-foreground">Filter functionality to be implemented</p>
                    </div>
                  </PopoverContent>
                </Popover>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      {visibleColumns.map((column) => (
                        <TableHead
                          key={column.id}
                          className={`cursor-grab ${dragOverColumn === column.id ? 'bg-accent' : ''}`}
                          draggable
                  ...
                  <TableBody>
                    {filteredDeliveries.length > 0 ? (
                      filteredDeliveries.slice(0, parseInt(rowsPerPage)).map((delivery) => (
                        <TableRow key={delivery.id}>
                          {visibleColumns.map((column) => (
                            <TableCell key={`${delivery.id}-${column.id}`}>
                              {column.id === "status" ? (
                                <Badge variant={getBadgeVariant(getStatusDisplay(delivery.status).color)}>
                                  {getStatusDisplay(delivery.status).name}
                                </Badge>
                              ) : column.id === "pickupLocation" || column.id === "dropoffLocation" ? (
                                <div>
                                  <div className="font-medium">{delivery[column.id].name}</div>
                                  <div className="text-xs text-muted-foreground">{delivery[column.id].address}</div>
                                </div>
                              ) : (
                                delivery[column.id]
                              )}
